...
CREATE TABLE IF NOT EXISTS currencyExchange.ExchangeRates(
    id serial PRIMARY KEY,
    base_currency_id int NOT NULL REFERENCES currencyExchange.Currencies(ID),
    target_currency_id int UNIQUE NOT NULL REFERENCES currencyExchange.Currencies(ID),
